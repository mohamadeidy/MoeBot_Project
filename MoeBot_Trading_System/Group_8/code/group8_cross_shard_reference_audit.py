...
references are audited separately by logical-sidecar generation.
"""
...
MAP={'price_action_pattern_candidate':('price_action_pattern_candidate','candidate_id'),'school_interpretation':('school_interpretation','interpretation_id'),'narrative_hypothesis':('narrative_hypothesis','hypothesis_id'),'shared_evidence':('shared_evidence','shared_evidence_id'),'conflicting_evidence':('conflicting_evidence','conflict_id'),'multi_timeframe_context_relation':('multi_timeframe_context_relation','relation_id')}
 ...
 try:
  ...
  for r in c.execute('SELECT shared_evidence_id,subject_ids_json FROM shared_evidence'):
   for rid in json.loads(r['subject_ids_json']):
    # shared evidence subjects can be interpretations or hypotheses; exactly one must resolve.

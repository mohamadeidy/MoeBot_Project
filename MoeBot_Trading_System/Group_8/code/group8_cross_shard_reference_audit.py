...
def audit(core_db:Path,pa7_catalog:Path,report:Path)->dict[str,Any]:
 ...
 try:
  ...
  for r in c.execute('SELECT shared_evidence_id,source_group,source_type,source_id,subject_ids_json FROM shared_evidence'):
   if str(r['source_group']).lower()=='group8':check(str(r['source_type']),r['source_id'],f"shared:{r['shared_evidence_id']}:source")
   for rid in json.loads(r['subject_ids_json']):
    if not exists_untyped_subject(str(rid)) and len(un)<100:un.append({'where':f"shared:{r['shared_evidence_id']}:subject",'target_type':'untyped_group8_subject','target_id':str(rid)})

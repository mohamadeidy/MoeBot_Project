...
from moebot_group8_engine_v0_8_0 import Group8Engine, Group8InvariantError, sha256_file
...
PA7_REQUIRED_TABLES: dict[str, tuple[str, ...]] = {
    "source": ("bars",),
    "group4": ("zones", "zone_transitions", "zone_interactions"),
    "group5": ("liquidity_pools",),
    "group6": ("fvg_events", "fvg_state_transitions", "imbalance_variants", "liquidity_voids", "bpr_relations"),
    "group7": ("institutional_zones", "zone_state_transitions"),
}
...
class _ExtremaTree:
    ...
    def __init__(self, bars: list[Any]) -> None:
        ...
        self.n = n; inf = float("inf")
        self.min_close=[inf]*(2*n); self.max_close=[-inf]*(2*n); self.min_high=[inf]*(2*n); self.max_low=[-inf]*(2*n); self.max_high=[-inf]*(2*n); self.min_low=[inf]*(2*n)
        for i,bar in enumerate(bars):
            p=n+i; self.min_close[p]=self.max_close[p]=float(bar.close); self.min_high[p]=self.max_high[p]=float(bar.high); self.max_low[p]=self.min_low[p]=float(bar.low)
        for p in range(n-1,0,-1):
            l,r=p*2,p*2+1; self.min_close[p]=min(self.min_close[l],self.min_close[r]); self.max_close[p]=max(self.max_close[l],self.max_close[r]); self.min_high[p]=min(self.min_high[l],self.min_high[r]); self.max_low[p]=max(self.max_low[l],self.max_low[r]); self.max_high[p]=max(self.max_high[l],self.max_high[r]); self.min_low[p]=min(self.min_low[l],self.min_low[r])
...
class PA7ShardEngine(Group8Engine):
    def __init__(self,*,spec:ShardSpec,**kwargs:Any)->None:
        self.shard_spec=spec;spec.validate();super().__init__(**kwargs)
        contract=json.loads((self.root/"SHARDED_STORAGE_CONTRACT.json").read_text());freeze=json.loads((self.root/"DESIGN_FREEZE_MANIFEST.json").read_text())
    ...
    def _verify_staging_contract(self)->None:
        manifest={r["key"]:r["value"] for r in self.input.execute("SELECT key,value FROM stage_manifest")}
        common={"year":str(self.year),"engine_version":self.config["engine_version"],"schema_version":self.config["schema_version"],"config_id":self.config["config_id"],"logical_dependency_lineage_id":EXPECTED_LOGICAL_LINEAGE,"adapter_map_hash":self.adapter["adapter_map_hash"]}
        ...
        tables=self._tables(self.input)
        compact=manifest.get("materialization_scope")=="PA7_COMPACT_V1"
        required_tables={"stage_manifest","staging_metadata"}
        if compact:
        ...
        if compact:
            identities=json.loads(manifest.get("database_identities_json","{}"))
            ...
            consumed=set(json.loads(manifest.get("materialized_groups_json","[]")))
    ...
    def process_failed_breakouts_and_retests_fast(self)->None:
        key=(self.shard_spec.symbol,self.shard_spec.timeframe);bars=self.bars_by_tf[key];tree=_ExtremaTree(bars);retest_cache:dict[tuple[int,float,float],int|None]={}
        sql="""SELECT * FROM price_action_pattern_candidate WHERE definition_id IN ('pa_breakout_exact','pa_breakout_point_buffer','pa_breakout_atr_buffer') AND symbol=? AND timeframe=? ORDER BY availability_time,candidate_id"""
        for br in self.out.execute(sql,key):
            src=self._bar_by_id(br["source_bar_id"])
            if src is None:continue
            _,idx=self.bar_pos[src.id];feats=json.loads(br["features_json"]);level=float(feats["locked_level"]);direction=str(br["direction"])

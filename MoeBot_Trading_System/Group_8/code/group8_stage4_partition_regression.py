...
LOGICAL_TABLES = (
    ...
    "narrative_hypothesis","hypothesis_lifecycle_event","shared_evidence",
    "conflicting_evidence","multi_timeframe_context_relation","evidence_chain",
    "invalidation_record","processing_checkpoint",
)
...
def _rows(con: sqlite3.Connection, table: str) -> list[dict[str,Any]]:
    ...
    order=",".join(f'"{c}"' for c in (_pk_cols(con,table) or cols))
    return [{c:row[i] for i,c in enumerate(cols)} for row in con.execute(f'SELECT * FROM "{table}" ORDER BY {order}')]


def _fingerprint(con: sqlite3.Connection, table: str) -> dict[str,Any]:
    rows=_rows(con,table);hashes=[hashlib.sha256(canonical_json(r).encode()).hexdigest() for r in rows];pk=_pk_cols(con,table)
    return {"count":len(rows),"primary_ids":[tuple(r[c] for c in pk) for r in rows],"row_hashes":hashes,"logical_fingerprint":stable_hash(hashes)}


def _counts_by_definition(con: sqlite3.Connection)->dict[str,dict[str,int]]:
    return {t:{str(r[0]):int(r[1]) for r in con.execute(f'SELECT definition_id,COUNT(*) FROM "{t}" GROUP BY definition_id ORDER BY definition_id')}
...
def _stage4_rows(con:sqlite3.Connection)->list[dict[str,Any]]:
    ...
    return [{c:row[i] for i,c in enumerate(cols)} for row in con.execute("SELECT * FROM price_action_pattern_candidate WHERE definition_id=? ORDER BY candidate_id",(STAGE4_DEF,))]
...
def _assert_zero_duplicates(con:sqlite3.Connection)->None:
    for table in LOGICAL_TABLES:
        ...
        if con.execute(f'SELECT 1 FROM "{table}" GROUP BY {group} HAVING COUNT(*)>1 LIMIT 1').fetchone() is not None:
            raise AssertionError(f"duplicate logical primary key:{table}")
    if con.execute("""SELECT 1 FROM price_action_pattern_candidate WHERE definition_id=?
      GROUP BY source_bar_id,event_time,confirmation_time,availability_time,features_json,upstream_refs_json HAVING COUNT(*)>1 LIMIT 1""",(STAGE4_DEF,)).fetchone() is not None:
        raise AssertionError("duplicate stage-4 domain output")


def _assert_references(con:sqlite3.Connection)->None:
    missing=int(con.execute("""SELECT COUNT(*) FROM price_action_pattern_state s LEFT JOIN price_action_pattern_candidate p
      ON p.candidate_id=s.candidate_id WHERE p.candidate_id IS NULL""").fetchone()[0])
...
def _fixture_ids(db:Path,per_partition:int)->set[str]:
    ...
    try:rows=con.execute("""SELECT candidate_id,availability_time FROM price_action_pattern_candidate
      WHERE definition_id IN (?,?) ORDER BY availability_time,candidate_id""",SOURCE_DEFS).fetchall()
    finally:con.close()
...
def _filtered_process(engine:AnnualCoreEngine,partition:int|None,ids:set[str])->dict[str,Any]|None:
    original=engine.out;_install_allowlist(original,ids)
    selected=original.execute("""SELECT * FROM price_action_pattern_candidate
      ...
      ORDER BY availability_time,candidate_id""").fetchall()
...
def _run_one_shot(staging:Path,base:Path,out:Path,root:Path,ids:set[str])->float:
    ...
    try:e.load_bars();started=time.monotonic();_filtered_process(e,None,ids);checkpoint(e,"context_rejections_fast");return time.monotonic()-started
    finally:e.close()
...
def _run_partitioned(staging:Path,base:Path,out:Path,root:Path,ids:set[str])->list[float]:
    ...
    try:
        ...
        for i in range(STAGE4_PARTITION_COUNT):
            _enforce_stage4_execution_order(e,i);started=time.monotonic();_filtered_process(e,i,ids);times.append(time.monotonic()-started)
...
def _negative_tests(staging:Path,base:Path,root:Path,ids:set[str],work:Path)->dict[str,str]:
    ...
    try:
        e.load_bars();_expect_fail("wrong execution order",lambda:_enforce_stage4_execution_order(e,1))
        _enforce_stage4_execution_order(e,0);r0=_filtered_process(e,0,ids)
        existing=_enforce_stage4_execution_order(e,0);r0b=_filtered_process(e,0,ids)
        ...
        _expect_fail("repeated partition",lambda:e.out.execute("INSERT INTO metadata(key,value) VALUES(?,?)",(key,e.out.execute("SELECT value FROM metadata WHERE key=?",(key,)).fetchone()[0])))
        ...
        bad=json.loads(saved);bad["plan_hash"]="0"*64;bad_payload={k:v for k,v in bad.items() if k!="receipt_hash"};bad["receipt_hash"]=stable_hash(bad_payload)
        e.out.execute("UPDATE metadata SET value=? WHERE key=?",(json.dumps(bad,sort_keys=True,separators=(',',':')),key))
        _expect_fail("wrong plan hash",lambda:_validated_receipt(e,0));e.out.execute("UPDATE metadata SET value=? WHERE key=?",(saved,key))
        bad=json.loads(saved);bad["receipt_hash"]="f"*64;e.out.execute("UPDATE metadata SET value=? WHERE key=?",(json.dumps(bad,sort_keys=True,separators=(',',':')),key))
        _expect_fail("conflicting receipt",lambda:_validated_receipt(e,0));e.out.execute("UPDATE metadata SET value=? WHERE key=?",(saved,key))
        _expect_fail("missing partition",e.verify_stage4_partition_coverage)
        clean=work/"negative_clean.sqlite";e.out.commit();shutil.copy2(db,clean)
...
def main()->int:
    ...
    p.add_argument("--fixture-per-partition",type=int,default=8);p.add_argument("--report",type=Path,required=True);a=p.parse_args()
    a.work_dir.mkdir(parents=True,exist_ok=True);ids=_fixture_ids(a.checkpoint2_db,a.fixture_per_partition);one=a.work_dir/"one_shot.sqlite";part=a.work_dir/"partitioned.sqlite"

...
Official closure remains release-gated. This stage additionally requires the
strengthened annual-validator identity and zero locked-context violations to be
stable across the frozen 2024 OOS boundary.
"""
...
def main() -> int:
    ...
    if oos.get("status") != "FROZEN_FOR_2024_OOS" or int(oos.get("format_version", 0)) < 2: failures.append("oos_freeze_v2_not_pass")
    if annual24.get("status") != "ANNUAL_2024_OOS_PASS" or int(annual24.get("format_version", 0)) < 2: failures.append("annual_2024_oos_v2_not_pass")
    if cross.get("status") != "PASS" or int(cross.get("format_version", 0)) < 2: failures.append("cross_year_v2_not_pass")
    if cross.get("identity_stable_across_oos_boundary") is not True: failures.append("identity_not_stable")
    if cross.get("locked_context_violations_both_years_zero") is not True: failures.append("locked_context_violations_cross_year")
    ...
    if not validator_sha: failures.append("annual_validator_identity_missing")
    ...
    if oos.get("annual_validator_sha256") != validator_sha or annual24.get("annual_validator_sha256") != validator_sha or cross.get("annual_validator_sha256") != validator_sha: failures.append("annual_validator_identity_drift")
    if oos.get("closed_blocking_gap", {}).get("gap_id") != LOCKED_CONTEXT_GAP_ID or annual24.get("closed_blocking_gap", {}).get("gap_id") != LOCKED_CONTEXT_GAP_ID or cross.get("closed_blocking_gap_id") != LOCKED_CONTEXT_GAP_ID: failures.append("locked_context_gap_provenance_drift")
    ...
    required = [
        ...
        "code/moebot_group8_engine_v0_8_0.py","code/group8_materialize_inputs.py","code/group8_postprocess_v0_8_0.py","code/group8_annual_validation.py",
        "tests/test_group8_engine_v0_8_0.py","tests/test_group8_lifecycle_persistence_v0_8_0.py",
        "reports/20_ENGINE_TECHNICAL_CANDIDATE_AUDIT.json","reports/27_LOCKED_CONTEXT_GAP_DIAGNOSTIC.json","reports/28_LOCKED_CONTEXT_GAP_ANALYSIS.json",
    ...
    audit = {
        "format_version":2,"status":"PASS","group":8,"phase":"FINAL_INDEPENDENT_AUDIT_PRE_RELEASE",
        "design_freeze_hash":design["design_freeze_hash"],"engine_build_manifest_hash":build["manifest_hash"],"engine_sha256":engine_sha,"annual_validator_sha256":validator_sha,"config_id":build["config_id"],
        ...
        "checks":{"design_frozen":True,"technical_candidate_pass":True,"2023_pass":True,"oos_freeze_pass":True,"2024_oos_pass":True,"cross_year_pass":True,"identity_stable":True,"annual_validator_stable":True,"locked_context_zero_both_years":True,"no_trading_outputs":True,"read_only_upstream":True,"annual_execution_revoked":True},
    ...
    next_group={
        "format_version":2,"status":"FROZEN_HANDOFF_PENDING_RELEASE_VERIFICATION","source_group":8,"source_group_name":status.get("name"),
        ...
        "design_freeze_hash":design["design_freeze_hash"],"engine_build_manifest_hash":build["manifest_hash"],"engine_sha256":engine_sha,"annual_validator_sha256":validator_sha,"closed_blocking_gap_id":LOCKED_CONTEXT_GAP_ID,
        ...
        "consumption_policy":{"read_only":True,"verify_sha256_before_use":True,"verify_release_tag":RELEASE_TAG,"do_not_rebuild_group8_semantics":True,"do_not_modify_group8_outputs":True,"preserve_causal_timestamps":True,"preserve_locked_context_fix":True,"trading_outputs_not_present":True},
    }
    ...
    candidate={"format_version":2,"status":"CLOSURE_CANDIDATE_RELEASE_REQUIRED","group":8,"release_tag":RELEASE_TAG,"engine_build_manifest_hash":build["manifest_hash"],"engine_sha256":engine_sha,"annual_validator_sha256":validator_sha,"closed_blocking_gap_id":LOCKED_CONTEXT_GAP_ID,"annual_2023_manifest_hash":annual23["manifest_hash"],"annual_2024_oos_manifest_hash":annual24["manifest_hash"],"cross_year_report_hash":cross["report_hash"],"final_audit_hash":audit["report_hash"],"next_group_dependency_manifest_hash":next_group["manifest_hash"],"officially_closed":False,"release_publication_verified":False}
    ...
    handoff=f"""# MoeBot Group 8 — Handoff Candidate\n\nGroup 8 has passed design, technical, 2023, frozen 2024 OOS, cross-year, and final pre-release audit gates. The annual-validator identity and locked-context correction `{LOCKED_CONTEXT_GAP_ID}` are frozen in the handoff. **It is not officially closed until the annual database assets are published and verified under GitHub Release `{RELEASE_TAG}`.**\n\nFuture groups must consume `NEXT_GROUP_DEPENDENCY_MANIFEST.json` read-only and verify every repository file/release asset before use.\n\nExpected release assets:\n- `{annual23['compressed_asset']['filename']}`\n- `{annual24['compressed_asset']['filename']}`\n""";a.handoff_output.write_text(handoff)

    status["officially_closed"]=False;status["next_group_start_authorized"]=False;status["closure_candidate"]={"status":candidate["status"],"candidate_hash":candidate["candidate_hash"],"final_audit_hash":audit["report_hash"],"next_group_dependency_manifest_hash":next_group["manifest_hash"],"annual_validator_sha256":validator_sha,"closed_gap_id":LOCKED_CONTEXT_GAP_ID,"required_release_tag":RELEASE_TAG};status["status"]="CLOSURE_CANDIDATE_RELEASE_REQUIRED";status_path.write_text(json.dumps(status,indent=2,sort_keys=True)+"\n")
    print(json.dumps({"status":candidate["status"],"candidate_hash":candidate["candidate_hash"],"annual_validator_sha256":validator_sha,"closed_gap_id":LOCKED_CONTEXT_GAP_ID,"release_tag":RELEASE_TAG,"officially_closed":False},indent=2,sort_keys=True));return 0

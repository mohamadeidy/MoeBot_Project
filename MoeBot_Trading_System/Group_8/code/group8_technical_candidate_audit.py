...
def main()->int:
    ...
    materializer_path=root/'code/group8_materialize_inputs.py'
    postprocessor_path=root/'code/group8_postprocess_v0_8_0.py'
    annual_validator_path=root/'code/group8_annual_validation.py'
    test_path=root/'tests/test_group8_engine_v0_8_0.py'
    lifecycle_test_path=root/'tests/test_group8_lifecycle_persistence_v0_8_0.py'
    gap_report_path=root/'reports/28_LOCKED_CONTEXT_GAP_ANALYSIS.json'
    engine_text=engine_path.read_text();materializer_text=materializer_path.read_text();postprocessor_text=postprocessor_path.read_text();annual_validator_text=annual_validator_path.read_text();lifecycle_test_text=lifecycle_test_path.read_text()
    future_refs=sorted(t for t in FORBIDDEN_TABLE_REFERENCES if t in engine_text or t in materializer_text or t in postprocessor_text)
    ...
    locked_context_markers={
        'shared_invalidator_helper':'first_bounded_range_invalidator' in postprocessor_text,
        'enumeration_horizon_guard':'bar.available_at>=invalidation_availability' in engine_text,
        'engine_locked_context_audit':'locked_context:ict_premium_discount_context' in engine_text,
        'annual_locked_context_audit':'locked_context_violations' in annual_validator_text and 'ict_premium_discount_context' in annual_validator_text,
        'permanent_regression':'test_ict_premium_discount_never_survives_locked_range_invalidation' in lifecycle_test_text,
        'gap_report_present':gap_report_path.is_file(),
    }
    checks['locked_context_hardening_present']=all(locked_context_markers.values())
    checks['locked_context_hardening_markers']=locked_context_markers
    if not checks['locked_context_hardening_present']:
        failures.append({'locked_context_hardening_missing':[k for k,v in locked_context_markers.items() if not v]})
    if gap_report_path.is_file():
        gap=json.loads(gap_report_path.read_text())
        checks['locked_context_gap_identity_valid']=(gap.get('gap_id')=='G8-ICT-LOCKED-CONTEXT-005' and gap.get('status')=='BLOCKING_GAP_FIXED_PENDING_TECHNICAL_REFREEZE' and gap.get('definitions_changed') is False and gap.get('thresholds_changed') is False and gap.get('schema_changed') is False and gap.get('upstream_changed') is False)
    else:
        checks['locked_context_gap_identity_valid']=False
    if not checks['locked_context_gap_identity_valid']:failures.append('locked_context_gap_identity_invalid')
    ...
    causal_markers=['availability_time < confirmation_time','confirmation_time < event_time','lifecycle before hypothesis availability','_active_zone_status_at','transition_time<=?','interaction_time<=?']
    ...
    hashes={
        'engine_sha256':file_hash(engine_path),
        'materializer_sha256':file_hash(materializer_path),
        'postprocessor_sha256':file_hash(postprocessor_path),
        'annual_validator_sha256':file_hash(annual_validator_path),
        'tests_sha256':file_hash(test_path),
        'lifecycle_tests_sha256':file_hash(lifecycle_test_path),
        'schema_sha256':file_hash(root/'02_SCHEMA.sql'),
        'definition_registry_file_sha256':file_hash(root/'01_DEFINITION_REGISTRY.json'),
        'frozen_config_file_sha256':file_hash(root/'FROZEN_CONFIG.json'),
        'locked_context_gap_analysis_sha256':file_hash(gap_report_path),
    }
    report={'format_version':2,'phase':'ENGINE_TECHNICAL_CANDIDATE_AUDIT','status':'PASS' if not failures else 'FAIL','engine_version':ENGINE_VERSION,'schema_version':SCHEMA_VERSION,'config_id':CONFIG_ID,'definition_count':len(frozen),'checks':checks,'hashes':hashes,'failures':failures}
    report['report_hash']=stable_hash(report);a.output.parent.mkdir(parents=True,exist_ok=True);a.output.write_text(json.dumps(report,indent=2,sort_keys=True)+'\n');print(json.dumps(report,indent=2,sort_keys=True));return 0 if not failures else 1

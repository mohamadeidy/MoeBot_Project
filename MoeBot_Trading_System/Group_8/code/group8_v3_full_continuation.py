...
def continue_all(*,work:Path,repo:Path,zstd:Path,d_temp_root:Path)->dict[str,Any]:
 ...
 if not s7u23.is_file():
  validate_stage7_union(release_path=s7rel23,plan_path=s7plan23,stage5_db=stage5_23,output_root=out7_23,work_root=work/"group8_v3_stage7_union_work",zstd_exe=zstd,output_path=s7u23,fan_in=16)
 ...
 if not freeze_path.is_file():
  freeze_oos(artifacts_root=art,annual_manifest_path=annual23,stage6_plan_path=s6plan23,stage7_plan_path=s7plan23,stage5_archive_report_path=s5arcr23,expected_commit=commit,output=freeze_path)
 fr=_load_hashed(freeze_path,"manifest_hash","FROZEN_FOR_2024_OOS_V3");_log("2024 OOS freeze PASS; first 2024 access now authorized")
 ...
 if not s7u24.exists():validate_stage7_union(release_path=s7rel24,plan_path=s7plan24,stage5_db=stage5_24,output_root=s7out24,work_root=oos/"stage7_union_work",zstd_exe=zstd,output_path=s7u24,fan_in=16)

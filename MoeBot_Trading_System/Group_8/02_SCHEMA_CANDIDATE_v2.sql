...
CREATE TABLE IF NOT EXISTS upstream_id_bridge(
...
CREATE INDEX IF NOT EXISTS ix_bridge_corrected_id
...
CREATE TRIGGER IF NOT EXISTS no_update_config_registry
BEFORE UPDATE ON config_registry BEGIN
    SELECT RAISE(ABORT,'immutable record: config_registry');
END;
CREATE TRIGGER IF NOT EXISTS no_delete_config_registry
BEFORE DELETE ON config_registry BEGIN
    SELECT RAISE(ABORT,'immutable record: config_registry');
END;
CREATE TRIGGER IF NOT EXISTS no_update_dataset_registry
BEFORE UPDATE ON dataset_registry BEGIN
    SELECT RAISE(ABORT,'immutable record: dataset_registry');
END;
CREATE TRIGGER IF NOT EXISTS no_delete_dataset_registry
BEFORE DELETE ON dataset_registry BEGIN
    SELECT RAISE(ABORT,'immutable record: dataset_registry');
END;
CREATE TRIGGER IF NOT EXISTS no_update_dependency_registry
BEFORE UPDATE ON dependency_registry BEGIN
    SELECT RAISE(ABORT,'immutable record: dependency_registry');
END;
CREATE TRIGGER IF NOT EXISTS no_delete_dependency_registry
BEFORE DELETE ON dependency_registry BEGIN
    SELECT RAISE(ABORT,'immutable record: dependency_registry');
END;
CREATE TRIGGER IF NOT EXISTS no_update_upstream_id_bridge
BEFORE UPDATE ON upstream_id_bridge BEGIN
    SELECT RAISE(ABORT,'immutable record: upstream_id_bridge');
END;
CREATE TRIGGER IF NOT EXISTS no_delete_upstream_id_bridge
BEFORE DELETE ON upstream_id_bridge BEGIN
    SELECT RAISE(ABORT,'immutable record: upstream_id_bridge');
END;
CREATE TRIGGER IF NOT EXISTS no_update_school_registry
BEFORE UPDATE ON school_registry BEGIN
    SELECT RAISE(ABORT,'immutable record: school_registry');
END;
CREATE TRIGGER IF NOT EXISTS no_delete_school_registry
BEFORE DELETE ON school_registry BEGIN
    SELECT RAISE(ABORT,'immutable record: school_registry');
END;
CREATE TRIGGER IF NOT EXISTS no_update_pattern_definition_registry
BEFORE UPDATE ON pattern_definition_registry BEGIN
    SELECT RAISE(ABORT,'immutable record: pattern_definition_registry');
END;
CREATE TRIGGER IF NOT EXISTS no_delete_pattern_definition_registry
BEFORE DELETE ON pattern_definition_registry BEGIN
    SELECT RAISE(ABORT,'immutable record: pattern_definition_registry');
END;
CREATE TRIGGER IF NOT EXISTS no_update_interpretation_definition_registry
BEFORE UPDATE ON interpretation_definition_registry BEGIN
    SELECT RAISE(ABORT,'immutable record: interpretation_definition_registry');
END;
CREATE TRIGGER IF NOT EXISTS no_delete_interpretation_definition_registry
BEFORE DELETE ON interpretation_definition_registry BEGIN
    SELECT RAISE(ABORT,'immutable record: interpretation_definition_registry');
END;
CREATE TRIGGER IF NOT EXISTS no_update_price_action_pattern_state
BEFORE UPDATE ON price_action_pattern_state BEGIN
    SELECT RAISE(ABORT,'immutable record: price_action_pattern_state');
END;
CREATE TRIGGER IF NOT EXISTS no_delete_price_action_pattern_state
BEFORE DELETE ON price_action_pattern_state BEGIN
    SELECT RAISE(ABORT,'immutable record: price_action_pattern_state');
END;
CREATE TRIGGER IF NOT EXISTS no_update_hypothesis_lifecycle_event
BEFORE UPDATE ON hypothesis_lifecycle_event BEGIN
    SELECT RAISE(ABORT,'immutable record: hypothesis_lifecycle_event');
END;
CREATE TRIGGER IF NOT EXISTS no_delete_hypothesis_lifecycle_event
BEFORE DELETE ON hypothesis_lifecycle_event BEGIN
    SELECT RAISE(ABORT,'immutable record: hypothesis_lifecycle_event');
END;
CREATE TRIGGER IF NOT EXISTS no_update_multi_timeframe_context_relation
BEFORE UPDATE ON multi_timeframe_context_relation BEGIN
    SELECT RAISE(ABORT,'immutable record: multi_timeframe_context_relation');
END;
CREATE TRIGGER IF NOT EXISTS no_delete_multi_timeframe_context_relation
BEFORE DELETE ON multi_timeframe_context_relation BEGIN
    SELECT RAISE(ABORT,'immutable record: multi_timeframe_context_relation');
END;
CREATE TRIGGER IF NOT EXISTS no_update_evidence_chain
BEFORE UPDATE ON evidence_chain BEGIN
    SELECT RAISE(ABORT,'immutable record: evidence_chain');
END;
CREATE TRIGGER IF NOT EXISTS no_delete_evidence_chain
BEFORE DELETE ON evidence_chain BEGIN
    SELECT RAISE(ABORT,'immutable record: evidence_chain');
END;
CREATE TRIGGER IF NOT EXISTS no_update_invalidation_record
BEFORE UPDATE ON invalidation_record BEGIN
    SELECT RAISE(ABORT,'immutable record: invalidation_record');
END;
CREATE TRIGGER IF NOT EXISTS no_delete_invalidation_record
BEFORE DELETE ON invalidation_record BEGIN
    SELECT RAISE(ABORT,'immutable record: invalidation_record');
END;
CREATE TRIGGER IF NOT EXISTS no_update_group8_audit_evidence
BEFORE UPDATE ON group8_audit_evidence BEGIN
    SELECT RAISE(ABORT,'immutable record: group8_audit_evidence');
END;
CREATE TRIGGER IF NOT EXISTS no_delete_group8_audit_evidence
BEFORE DELETE ON group8_audit_evidence BEGIN
    SELECT RAISE(ABORT,'immutable record: group8_audit_evidence');
END;

...
class OOSCrossYearClosureTests(unittest.TestCase):
    ...
    @staticmethod
    def _freeze(build, a23):
        return {
            "format_version":2,"status":"FROZEN_FOR_2024_OOS","manifest_hash":"ooshash","annual_2023_manifest_hash":a23["manifest_hash"],
            "engine_build_manifest_hash":build["manifest_hash"],"engine_sha256":a23["engine_sha256"],"postprocessor_sha256":a23["postprocessor_sha256"],"materializer_sha256":a23["materializer_sha256"],
            "annual_validator_sha256":build["identities"]["annual_validator"]["sha256"],"config_id":build["config_id"],
            "closed_blocking_gap":{"gap_id":GAP_ID,"2023_locked_context_violations":0},"identities":{},
        }

    def test_2024_oos_finalizer_revokes_execution_and_binds_validator(self):
        with tempfile.TemporaryDirectory() as td:
            root=Path(td);build,status,a23=self._base(root);freeze=self._freeze(build,a23);write(root/"OOS_FREEZE_MANIFEST.json",freeze)
            ...
            annual_base={"status":"PASS","year":2024,"engine_version":build["engine_version"],"schema_version":build["schema_version"],"config_id":build["config_id"],"engine_build_manifest_hash":build["manifest_hash"],"engine_sha256":a23["engine_sha256"],"postprocessor_sha256":a23["postprocessor_sha256"],"locked_context_violations":0,"causality_errors":{},"upstream_reference_integrity":{"unresolved_group8":0,"unresolved_upstream":0,"unknown_source_types":[]},"lifecycle":{"hypotheses":1,"initial":1,"terminal":1,"before_creation":0},"counts":{"narrative_hypothesis":1},"distributions":{"hypotheses_by_school":{"school":1}},"no_trading_outputs":True,"read_only_upstream":True,"failures":[]}
            annuals=[];fps=[]
            ...
            m=json.loads(out.read_text());s=json.loads((root/"STATUS.json").read_text());self.assertEqual(m["format_version"],2);self.assertEqual(m["status"],"ANNUAL_2024_OOS_PASS");self.assertEqual(m["annual_validator_sha256"],build["identities"]["annual_validator"]["sha256"]);self.assertEqual(m["locked_context_violations"],0);self.assertFalse(m["frozen_identity_drift"]);self.assertFalse(s["annual_execution_authorized"]);self.assertFalse(s["annual_execution_2024_authorized"]);self.assertEqual(s["status"],"ANNUAL_2024_OOS_PASS_CROSS_YEAR_REQUIRED")

    def test_cross_year_accepts_descriptive_frequency_drift_only_when_locked_context_zero(self):
        with tempfile.TemporaryDirectory() as td:
            root=Path(td);build,status,a23=self._base(root);freeze=self._freeze(build,a23);write(root/"OOS_FREEZE_MANIFEST.json",freeze)
            a24={**a23,"format_version":2,"status":"ANNUAL_2024_OOS_PASS","year":2024,"manifest_hash":"annual24hash","oos_freeze_manifest_hash":"ooshash","logical_fingerprint":"logical24","annual_validator_sha256":build["identities"]["annual_validator"]["sha256"],"closed_blocking_gap":{"gap_id":GAP_ID},"locked_context_violations":0};write(root/"ANNUAL_2024_OOS_VALIDATION_MANIFEST.json",a24);status["annual_execution_authorized"]=False;status["annual_execution_2023_authorized"]=False;status["annual_execution_2024_authorized"]=False;status["status"]="ANNUAL_2024_OOS_PASS_CROSS_YEAR_REQUIRED";write(root/"STATUS.json",status)
            base={"status":"PASS","failures":[],"no_trading_outputs":True,"read_only_upstream":True,"locked_context_violations":0,"causality_errors":{},"upstream_reference_integrity":{"unresolved_group8":0,"unresolved_upstream":0,"unknown_source_types":[]},"lifecycle":{"hypotheses":2,"initial":2,"terminal":2,"before_creation":0}}
            ...
            r=json.loads(out.read_text());self.assertEqual(r["format_version"],2);self.assertTrue(r["identity_stable_across_oos_boundary"]);self.assertTrue(r["locked_context_violations_both_years_zero"]);self.assertEqual(r["annual_validator_sha256"],build["identities"]["annual_validator"]["sha256"]);self.assertEqual(r["counts"]["x"]["2023"],10);self.assertEqual(r["counts"]["x"]["2024"],25)

    def test_release_gated_official_closure_preserves_validator_and_gap(self):
        with tempfile.TemporaryDirectory() as td:
            root=Path(td);build,status,a23=self._base(root);oos=self._freeze(build,a23);write(root/"OOS_FREEZE_MANIFEST.json",oos)
            a24={**a23,"format_version":2,"status":"ANNUAL_2024_OOS_PASS","year":2024,"manifest_hash":"annual24hash","oos_freeze_manifest_hash":"ooshash","logical_fingerprint":"logical24","annual_validator_sha256":build["identities"]["annual_validator"]["sha256"],"closed_blocking_gap":{"gap_id":GAP_ID},"locked_context_violations":0,"database":{"filename":"g8_2024.sqlite","size_bytes":11,"sha256":"db24","logical_sha256":"logical24"},"compressed_asset":{"filename":"g8_2024.sqlite.zst","size_bytes":6,"sha256":"z24"},"validation_run_id":"222","artifact_name":"artifact24"};write(root/"ANNUAL_2024_OOS_VALIDATION_MANIFEST.json",a24)
            cross=rh({"format_version":2,"status":"PASS","annual_validator_sha256":build["identities"]["annual_validator"]["sha256"],"closed_blocking_gap_id":GAP_ID,"identity_stable_across_oos_boundary":True,"locked_context_violations_both_years_zero":True,"no_trading_outputs_both_years":True,"read_only_upstream_both_years":True});write(root/"reports/50_CROSS_YEAR_VALIDATION.json",cross);status["status"]="ANNUAL_2024_OOS_PASS_CROSS_YEAR_REQUIRED";status["annual_execution_authorized"]=False;status["annual_execution_2023_authorized"]=False;status["annual_execution_2024_authorized"]=False;write(root/"STATUS.json",status)
            required=["00_DESIGN_LOCK.md","01_DEFINITION_REGISTRY.json","02_SCHEMA.sql","contracts/UPSTREAM_INPUT_CONTRACT.json","UPSTREAM_ANNUAL_DEPENDENCY_REGISTRY.json","UPSTREAM_ADAPTER_MAP.json","UPSTREAM_VALUE_BINDINGS.json","UPSTREAM_REFERENCE_RESOLUTION.json","code/moebot_group8_engine_v0_8_0.py","code/group8_materialize_inputs.py","code/group8_postprocess_v0_8_0.py","code/group8_annual_validation.py","tests/test_group8_engine_v0_8_0.py","tests/test_group8_lifecycle_persistence_v0_8_0.py","reports/20_ENGINE_TECHNICAL_CANDIDATE_AUDIT.json","reports/27_LOCKED_CONTEXT_GAP_DIAGNOSTIC.json","reports/28_LOCKED_CONTEXT_GAP_ANALYSIS.json","reports/30_ANNUAL_2023_MATERIALIZATION.json","reports/31_ANNUAL_2023_ENGINE_AUDIT.json","reports/32_ANNUAL_2023_VALIDATION.json","reports/33_ANNUAL_2023_OUTPUT_FINGERPRINT.json","reports/34_ANNUAL_2023_CLEAN_RECONSTRUCTION.json","reports/40_ANNUAL_2024_MATERIALIZATION.json","reports/41_ANNUAL_2024_ENGINE_AUDIT.json","reports/42_ANNUAL_2024_OOS_VALIDATION.json","reports/43_ANNUAL_2024_OUTPUT_FINGERPRINT.json","reports/44_ANNUAL_2024_CLEAN_RECONSTRUCTION.json"]
            for rel in required:
                p=root/rel
            ...
            self.assertEqual(n["format_version"],2);self.assertEqual(n["annual_validator_sha256"],build["identities"]["annual_validator"]["sha256"]);self.assertEqual(n["closed_blocking_gap_id"],GAP_ID);self.assertTrue(n["consumption_policy"]["preserve_locked_context_fix"])

...
class PA7AnnualLifecycleParity(unittest.TestCase):
    def test_scoped_shard_union_matches_full_reference_candidates_and_all_states(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            expc=cmap(refdb);exps=smap(refdb,set(expc));self.assertGreater(len(expc),0)
            gotc={};gots={}
            for tf in ('M15','H1'):
                for scope in ('upstream','group8_range'):
                    for bucket in range(4):
                        ...
                        cm=cmap(out);sm=smap(out,set(cm))
                        for k,v in cm.items():self.assertNotIn(k,gotc);gotc[k]=v
                        for k,v in sm.items():self.assertNotIn(k,gots);gots[k]=v
            self.assertEqual(expc,gotc)
            self.assertEqual(exps,gots)
            # Reference postprocessing must add at least one ordinal>0 PA7 state,
            # otherwise this test would not exercise annual follow-up finalization.
            con=sqlite3.connect(refdb)
            try:
                q=','.join('?' for _ in expc);n=0
                ids=sorted(expc)
